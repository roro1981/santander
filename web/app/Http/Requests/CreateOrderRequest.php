...
class CreateOrderRequest extends CustomFormRequest
{
    public function rules(): array
    {
        ...
        return [
            'uuid' => [self::REQUIRED, self::STRING, self::MAX_255, self::UUID],
            'order.id' => self::REQUIRED . '|' . self::NUMERIC,
            'order.product_id' => self::REQUIRED . '|' . self::NUMERIC,
            'order.method_id' => self::NUMERIC,
            ...
            'user.legal_name' => self::STRING . '|' . self::MAX_255,
            'user.tax_id' => self::STRING . '|' . self::MAX_255,
            'user.address' => self::STRING . '|' . self::MAX_255,
