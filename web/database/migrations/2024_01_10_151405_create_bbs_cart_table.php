...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('bbs_cart', function (Blueprint $table) {
            ...
            $table->integer('car_id_transaction')->comment('Identificador transaccion');
            $table->string('car_currency', 3)->comment('Moneda del cobro');
            $table->string('car_amount', 18)->comment('Monto total para pagar');
            $table->string('car_description', 100)->comment('Descripción del ítem del carro');
            $table->string('car_agreement', 20)->comment('Codigo de convenio asociado al comercio');
            ...
            $table->string('car_collector', 20)->comment('Codigo identificador del recaudador');
            ...
            $table->string('car_authorization_uuid', 255)->comment('Codigo autorización de orden entregada por el webhook');
            $table->tinyInteger('car_sent_kafka')->length(1)->comment('Verifica si carro fue enviado a kafka');
            $table->string('car_fail_code', 255)->comment('Codigo error webhook');
            $table->longText('car_fail_motive')->comment('Detalle error webhook');
            ...
            $table->timestamp('car_created_at')->comment('Fecha creación');
            $table->timestamp('car_updated_at')->comment('Fecha modificación');
        });
    }
